...
function addUser($voornaam, $achternaam, $telefoon, $geslacht, $email, $logonNaam, $wachtwoord){
    ...
    $sql = "INSERT INTO users (user_voornaam, user_achternaam, user_telefoonnummer, user_geslacht, user_email) VALUES (?, ?,?,?,?);";
    

    $sql2 = "INSERT INTO user_logon (user_login_naam, user_wachtwoord) VALUES (?,?);";

    
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('sssss', $voornaam, $achternaam, $telefoon, $geslacht, $email);
	$stmt->execute();

    
    $stmt2 = $conn->prepare($sql2);
    $stmt2->bind_param('ss', $logonNaam, $wachtwoord);
	$stmt2->execute();
